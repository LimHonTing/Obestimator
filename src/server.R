...
shinyServer(function(input, output, session) {
    ...
    output$plot1 <- renderPlot({
      ...
      ggplot(df,aes(x=NObeyesdad.cat,fill=NObeyesdad.cat)) + 
        geom_bar() +
        labs(x="Obesity Level Category",y="Count",fill="Obesity Level Category") +
    ...
    output$plot2 <- renderPlot({
      ...
      ggplot(df,aes(x=NObeyesdad.cat,fill=FAF.cat)) + 
        geom_bar(position = "dodge") +
        labs(x="Obesity Level Category",fill="How often do you have physical activity?") +
